...
export const markAsHelpful = async (req, res) => {
  ...
  try {
    const response = await Review.findByIdAndUpdate(reviewId, { $inc: { helpfulCount: 1 } });
    return res.status(200).json({ message: "Helpful count updated Successfully" })
  } catch (error) {
    console.log(error)
    return res.status(500).json({ error: "Server Error while updtaing review" })
  }
}
...
export const getReviews = (sortBy) => async (req, res) => {
  ...
  const data = (sortBy === "recent") ? await Review
    .find({ apartment: apartmentId })
    .sort({ datePosted: -1 })
    .exec()
    : await Review
      .find({ apartment: apartmentId })
